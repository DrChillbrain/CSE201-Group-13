...
db.serialize(() => {
  ...
  db.run(
    'create TABLE playlists(playlist_id INTEGER PRIMARY KEY AUTOINCREMENT, playlist_name VARCHAR(255), playlist_description VARCHAR(2000), user_id integer REFERENCES users (id), public BOOLEAN)'
  );
  db.run(
  ...
  db.run(
    'INSERT INTO songs(name, genre, artist) VALUES ("Born to Run", "Rock", "Bruce Springsteen")'
  );
  db.run(
